...
function CourseRoutes(app, connection) {
	...
	app.route('/Course/numbers/').get((req, res) => {
		const query = `SELECT DISTINCT course_number FROM Course WHERE subject="${req.query.subject}";`
		console.log(query);
		connection.query(query, (error, results, fields) => {
			if (error || results.length == 0) {
				return res.sendStatus(500);
	...
	app.route('/Course/insert/').get((req, res) => {
		...
		connection.query(CourseQuery, (error, results, fields) => {
			...
			`"${req.query.subject}", "${req.query.course_number}", "${req.query.name}", "${req.query.description}");`
			...
			connection.query(query, (error, results, fields) => {
				...
				preReqs.map(prereq => `("${req.query.subject}", "${req.query.course_number}", "${prereq.subject}", "${prereq.course_number}")`).join(', ') + ';';
				connection.query(InsertPreReq, (error, results, fields) => {
					...
					antiReqs.map(antireq => `("${req.query.subject}", "${req.query.course_number}", "${antireq.subject}", "${antireq.course_number}")`).join(', ') + ';';
	...
	app.route('/Course/prereqs/insert').get((req, res) => {
		const query = `INSERT INTO PreRequisites (subject, course_number, pre_requisite_subject, pre_requisite_number) VALUES (` +
						`"${req.query.subject}", "${req.query.course_number}", "${req.query.prereq_subject}", "${req.query.prereq_number}");`
	...
	app.route('/Course/antireqs/insert').get((req, res) => {
		const query = `INSERT INTO AntiRequisites (subject, course_number, anti_requisite_subject, anti_requisite_number) VALUES (` +
						`"${req.query.subject}", "${req.query.course_number}", "${req.query.antireq_subject}", "${req.query.antireq_number}");`
	...
	app.route('/Course/delete/').get((req, res) => {
		const query = `DELETE FROM Course WHERE subject = "${req.query.subject}" AND course_number = "${req.query.course_number}";`
	...
	app.route('/Course/prereqs/').get((req, res) => {
		const query = `CALL GetPreReqs("${req.query.subject}", "${req.query.course_number}");`
	...
	app.route('/Course/antireqs/').get((req, res) => {
		const query = `CALL GetAntiReqs("${req.query.subject}", "${req.query.course_number}");`
	...
	app.route('/Course/search/').get((req, res) => {
		...
		if (req.query.availableOnly == "true") {
			if (req.query.subject) {
				query = `CALL SearchAvailableSubject("${req.query.subject}", "%${req.query.search}%");`
			} else {
				query = `CALL SearchAvailable("%${req.query.search}%");`
			}
		} else {
			if (req.query.subject) {
				query = `CALL SearchSubject("${req.query.subject}", "%${req.query.search}%");`
			} else {
				query = `CALL Search("%${req.query.search}%");`
			}
		}
